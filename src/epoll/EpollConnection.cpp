...
EpollConnection::EpollConnection(int fd, DataGrid &dataGrid, ThreadPool &resourcePool)
    :   dataGrid(dataGrid),
        resourcePool(resourcePool),
        messageInProgress(false),
        messageBuffer(nullptr),
        inProgerssMessageSize(0),
        inProgerssMessageRead(0)
{
...
void EpollConnection::writeResponse(esw::Response &response) {
    ...
    if (bytesSent < 0) {
        throw runtime_error("[CONNECTION " + to_string(this->get_fd()) + "][ERROR]  Failed to send response size: " + string(strerror(errno)));
    }
...
bool EpollConnection::readEvent() {
    ...
    if (!messageInProgress) {
        // New message
        inProgerssMessageSize = readMessageSize();
        if (inProgerssMessageSize < 0) {
            return CLOSE_CONNECTION;
        }

        inProgerssMessageRead = 0;
        messageBuffer = new char[inProgerssMessageSize];
    }

    // Message in progress
    int received = recv(this->get_fd(), messageBuffer + inProgerssMessageRead, inProgerssMessageSize - inProgerssMessageRead, MSG_WAITALL);\
    ...
    inProgerssMessageRead += received;
    #ifdef DEBUG_CONNECTION
    cout << "[CONNECTION " << this->get_fd() << "]  Received: " << received << endl;
    #endif

    // Was everything read from the socket?
    if (inProgerssMessageRead != inProgerssMessageSize) {
        #ifdef DEBUG_CONNECTION
        cout << "[CONNECTION " << this->get_fd() << "]  Message in progress (recieved " << inProgerssMessageRead << " out of " << inProgerssMessageSize << " )" << endl;
        #endif
    ...
    esw::Response response;
    response.set_status(esw::Response_Status_OK);
    request.ParseFromArray(messageBuffer, inProgerssMessageSize);
    ...
    if (request.has_walk()) {
        ...
        uint64_t val = dataGrid.processOneToOne(oneToOne);
        ...
        uint64_t val = dataGrid.processOneToAll(oneToAll);
