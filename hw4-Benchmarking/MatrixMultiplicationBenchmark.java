...
public class MatrixMultiplicationBenchmark {
    ...
    // public double[][] measureMultiply(StateVars state) {
    //     return MatrixUtils.multiply(state.a, state.b);
    // }

    @Benchmark
//    @Warmup(iterations = 10, time = 3, timeUnit = TimeUnit.SECONDS)
    public void measureMultiply(StateVars state, Blackhole blackhole) {
        blackhole.consume(MatrixUtils.multiply(state.a, state.b));
    }

    // @Benchmark
    // @Warmup(iterations = 10, time = 3, timeUnit = TimeUnit.SECONDS)
    // public double [] measureMultiply1D(StateVars state) {
    //     return MatrixUtils.multiply1D(state.a1D, state.b1D, N, M, P);
    // }

    @Benchmark
//    @Warmup(iterations = 10, time = 3, timeUnit = TimeUnit.SECONDS)
    public void measureMultiply1D(StateVars state, Blackhole blackhole) {
        blackhole.consume(MatrixUtils.multiply1D(state.a1D, state.b1D, N, M, P));
    }

    // @Benchmark
    // @Warmup(iterations = 10, time = 3, timeUnit = TimeUnit.SECONDS)
    // public double [][] measureMultiplyTrans(StateVars state) {
    //     return MatrixUtils.multiplyTrans(state.a, state.b);
    // }

    @Benchmark
//    @Warmup(iterations = 10, time = 3, timeUnit = TimeUnit.SECONDS)
    public void measureMultiplyTrans(StateVars state, Blackhole blackhole) {
        blackhole.consume(MatrixUtils.multiplyTrans(state.a, state.b));
    }
